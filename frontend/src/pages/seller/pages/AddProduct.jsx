...
import { useEffect, useState } from 'react';
import { Box, CircularProgress, Stack, TextField, FormControlLabel, Radio, RadioGroup, FormControl, FormLabel } from '@mui/material';
import Popup from '../../../components/Popup';
import { BlueButton } from '../../../utils/buttonStyles';
import { useDispatch, useSelector } from 'react-redux';
import { addStuff } from '../../../redux/userHandle';
...
const AddProduct = () => {
  ...
  const [description, setDescription] = useState("");
  const [tagline, setTagline] = useState("");
  const [seller, setSeller] = useState(currentUser._id);
            ...
            <Stack spacing={1} sx={{ mb: 3 }}>
              {productImages.length > 0 ? (
                productImages.map((imageUrl, index) => (
                  <ProductImage key={index} src={imageUrl} alt="" />
                ))
              ) : (
                <ProductImage src={altImage} alt="" />
              )}
            ...
            <form onSubmit={submitHandler}>
              <Stack spacing={3}>
                <FormControl component="fieldset">
                  ...
                  <RadioGroup row aria-label="upload method" name="upload-method" value={uploadMethod} onChange={(event) => setUploadMethod(event.target.value)}>
                    <FormControlLabel value="url" control={<Radio />} label="From URL" />
                    <FormControlLabel value="local" control={<Radio />} label="From Local Device" />
                  </RadioGroup>
                </FormControl>
                {uploadMethod === 'url' ? (
                  <TextField
